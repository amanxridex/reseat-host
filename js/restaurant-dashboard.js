...
// Load restaurants from backend (cookie automatically sent)
async function loadRestaurants() {
    try {
        // TODO: Replace with real endpoint when available
        // const res = await fetch(`${API_URL}/host/restaurants`, {
        //     credentials: 'include', // ✅ Cookie sent
        //     headers: { 'Content-Type': 'application/json' }
        // });
        
        const container = document.getElementById('liveRestaurants');
        container.innerHTML = `
            <div class="empty-state" style="grid-column: 1/-1; text-align: center; padding: 3rem;">
                <div style="font-size: 4rem; margin-bottom: 1rem;">🎉</div>
                <h3>No Restaurants Yet</h3>
                <p style="color: var(--text-muted); margin-bottom: 1.5rem;">Create your first college restaurant!</p>
                <a href="create-restaurant.html" class="btn-primary" style="display: inline-block; padding: 0.75rem 1.5rem; text-decoration: none;">
                    Create Restaurant
                </a>
            </div>
        `;
...
function shareTo(platform) {
    const url = 'https://nexus.app/restaurant/techrestaurant-2025';
    
    if (platform === 'whatsapp') {
        window.open(`https://wa.me/?text=Check out this restaurant! ${url}`);
...
function openRestaurant(restaurantId) {
    sessionStorage.setItem('nexus_current_restaurant', restaurantId);
